...
public class RiskaMenuSliderInt extends RiskaMenuItem {
	...
	public RiskaMenuSliderInt(float pX, float pY, int minBound, int maxBound, int pStep, RiskaSprite pBackground, RiskaSprite pSlider)
	{
		super(pX, pY);
		...
		this.min = minBound / pStep + 1;
		this.max = maxBound / pStep + 1;
		this.step = pStep;
	...
	public boolean onAreaTouched(TouchEvent ev, float pX, float pY) {
		...
		case MotionEvent.ACTION_UP:
			if(Utils.contains(this, pX, pY))
			{
				...
				float x = (value / max) * getWidth();
				
				this.slider.setPosition(x, slider.getY());
			}
			break;
		}
